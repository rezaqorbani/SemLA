...
if __name__ == "__main__":
    ...
    for name in ["train"]:
        ...
        val_list = [
            ...
            for f in np.loadtxt(osp.join(voc_path, "ImageSets/Segmentation/train.txt"), dtype=str).tolist()
        ]
        for file in tqdm.tqdm(val_list):
            convert_to_trainID(file, out_mask_dir, is_train=True)
            convert_to_trainID(file, out_mask_dir_bg, is_train=True, clsID_to_trID=clsID_to_trID_bg)