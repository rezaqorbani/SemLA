...
def _get_acdc_files(image_dir, gt_dir):
    ...
    for city in cities:
        ...
        for basename in PathManager.ls(city_img_dir):
            ...
            suffix = "rgb_anon.png"
            suffix_ref = "rgb_ref_anon.png"
            assert (basename.endswith(suffix) or basename.endswith(suffix_ref)), basename
            if basename.endswith(suffix):
                ...
                basename = basename[: -len(suffix_ref)]
                
                instance_file = os.path.join(
                    city_gt_dir, basename + "gtFine_instanceIds.png"
                )
                label_file = os.path.join(city_gt_dir, basename + "gt_ref_labelIds.png")
                json_file = os.path.join(city_gt_dir, basename + "gtFine_polygons.json")
...
def load_acdc_semantic(image_dir, gt_dir):
    """
    Args:
    ...
    for image_file, label_file in _get_acdc_files(
        image_dir, gt_dir
    ):
